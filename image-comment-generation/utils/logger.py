...
class Logger:
    ...
    def update(self, stats):
        ...
        if self.i % self.log_per_updates == 0:
            ...
            stats_str = ' '.join(f'{key}[{val:.3f}]' for key, val in stats.items())
    ...
    def log_eval(self, stats, metrics_group):
        stats_str = ' '.join(f'{key}: {val:.3f}' for key, val in stats.items())
