...
from utils.functional import str2bool
...
class Trainer(object):
    ...
    def _initialize_parser():
        parser = argparse.ArgumentParser('Training Image-Text Matching Model')
        ...
        training.add_argument('-gc', '--grad-clipping', metavar='x', type=float, default=5)
    ...
    def train(self):
        ...
        train, _ = load_data(self.args.data_dir, 'train', self.args.batch_size, self.args.distributed)
        ...
        for epoch in range(start_epoch, self.args.epochs + 1):
            ...
            step = 0
            for images, captions, target in train:
                stats = model.update(images, captions, target)
                self.log.update(stats)
                # Evaluate the model every 10000 step
                # if (step + 1) % 10_000 == 0:
                #     stats = model.validate(valid)
                #     self.log.log_eval(stats)
                # step += 1
...
if __name__ == '__main__':
    ...
    # python train.py -d data -o output -bs 32 --start-cnn-tuning 0 --ft-start-layer -16 --tensorboard
